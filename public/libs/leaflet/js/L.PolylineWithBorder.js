...
L.GeoJSONWithBorder = L.GeoJSON.extend({
    addData: function (geojson) {
        ...
        var layer = L.GeoJSONWithBorder.geometryToLayer(geojson, options);
        if (!layer) {
            return this;
        }
        layer.feature = L.GeoJSONWithBorder.asFeature(geojson);
...
L.GeoJSONWithBorder.geometryToLayer = function(geojson, options) {
    var geometry = geojson.type === 'Feature' ? geojson.geometry : geojson,
        coords = geometry ? geometry.coordinates : null,
        layers = [],
        pointToLayer = options && options.pointToLayer,
        _coordsToLatLng = options && options.coordsToLatLng || L.GeoJSONWithBorder.coordsToLatLng,
    ...
    switch (geometry.type) {
        ...
        case 'MultiLineString':
            latlngs = L.GeoJSONWithBorder.coordsToLatLngs(coords, geometry.type === 'LineString' ? 0 : 1, _coordsToLatLng);
            return new L.PolylineWithBorder(latlngs, options);

        case 'Polygon':
        case 'MultiPolygon':
            latlngs = L.GeoJSONWithBorder.coordsToLatLngs(coords, geometry.type === 'Polygon' ? 1 : 2, _coordsToLatLng);
            return new L.Polygon(latlngs, options);

        case 'GeometryCollection':
            for (i = 0, len = geometry.geometries.length; i < len; i++) {
                var layer = L.GeoJSONWithBorder.geometryToLayer({
